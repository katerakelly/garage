...
class PixelObservationWrapper(gym.Wrapper):
    ...
    def __init__(self, env, headless=True):
        ...
        env.reset()
        env = GymPixelObservationWrapper(env, render_kwargs={'pixels': {'mode':'rgb_array', 'width':64, 'height':64}}, pixels_only=False)
        super().__init__(env)
    ...
    def step(self, action):
        ...
        info['state'] = obs['state'] # put true state in env info
...
class GymPixelObservationWrapper(ObservationWrapper):
    """
    NOTE: this is copied from gym `master` branch as of 9/11/20
    it fixes a bug in previous versions of gym (0.15.4 incl.)
    that occurs when `pixels_only=False`
