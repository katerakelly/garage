...
from garage.torch.algos import InverseMI, StateDecoder
...
def main(rb, env, image, discrete, name, seed, gpu, debug, overwrite):
    ...
    def train_inverse(ctxt, rb, env, image, discrete, seed, gpu):
        ...
        state_dim=4
        mlp_encoder = MLPModule(input_dim=obs_dim,
        ...
        # load saved rb
        print('Loading replay buffer: {}...'.format(rb))
        ...
        test_info = replay_buffer.sample_transitions(batch_size=1)['env_info']
        print(test_info)

        # construct algo and train!
        algo = StateDecoder(predictor,
                            replay_buffer,
                            lr=1e-2,
